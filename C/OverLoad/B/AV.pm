...
sub save {
    ...
    if ( $fill > -1 and defined $magic and $magic !~ /D/ ) {
        ...
        for ( my $i = 0; $i <= $#array; $i++ ) {
            ...
            elsif ($use_padname_zero_speedup
                && defined $values[$i]
                && defined $values[ $i + 1 ]
                && defined $values[ $i + 2 ]
                && $values[$i] eq '&padname_list[0]'
                && $values[ $i + 1 ] eq '&padname_list[0]'
                && $values[ $i + 2 ] eq '&padname_list[0]' ) {
                $count = 0;
                while ( defined $values[ $i + $count + 1 ] and $values[ $i + $count + 1 ] eq '&padname_list[0]' ) {
                    $count++;
                }
        ...
        # might be faster also.
            ...
            if ( $fill > -1 && !$MYMALLOC) {
                init()->add(sprintf("\tSV **svp = INITAv($sym, %d);", $fill1 ));
            } else {
                init()->add(
                 "\tSV **svp;",
                 "\tAV *av = $sym;"
