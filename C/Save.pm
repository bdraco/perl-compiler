...
sub savepvn {
    ...
    if ( defined $max_string_len && length($pv) > $max_string_len ) {
        ...
        if ( $sv and ( ( $sv->FLAGS & 0x09000000 ) == 0x09000000 ) ) {
            ...
            if ( $cur && !B::C::IsREADONLY($sv) && $dest =~ m{sv_list\[([^\]]+)\]\.} && $len < $max_string_len && ( !$seencow{$cstr} || $seencow{$cstr}->[1] < 255 ) ) {    # 1 was B::C::IsCOW($sv)
                my $svidx = $1;
                debug( sv => "COW: Saving PV %s:%d to %s", $cstr, $cur, $dest );
                push @init, sprintf( "%s = %s;", $dest, cowpv($pv) );
                push @init, sprintf( "SvFLAGS(&sv_list[%d]) |= SVf_IsCOW | SVf_IsCONST | SVf_THINKFIRST;", $svidx );
